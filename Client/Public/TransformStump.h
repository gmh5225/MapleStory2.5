...
class CTransformStump final : public CCreature
{
private:
	enum RandomMove { MOVE_R, MOVE_L, MOVE_U, MOVE_D, MOVE_END };
private:
	CTransformStump(LPDIRECT3DDEVICE9 pGraphic_Device);
	CTransformStump(const CTransformStump& rhs);
	virtual ~CTransformStump() = default;
...
public:
	static CTransformStump* Create(LPDIRECT3DDEVICE9 pGraphic_Device);
